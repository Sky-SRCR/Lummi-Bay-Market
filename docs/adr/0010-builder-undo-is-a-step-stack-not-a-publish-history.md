...
It is stored as the ninth entry in `BrandingConfig::DEFAULTS`, which is to say in
`branding_config.php` alongside the store name and the nav colours, rather than as a
`define()` of its own. That is not filing convenience: `BrandingConfig` is the only
writer of that file and rewrites it whole from its own list, so a setting declared
anywhere else would be dropped the next time somebody saved the Branding form — the
value gone, the form saying "Settings saved." `undoStepsSetting()` in `config.php` is
the one reader that turns the stored string into a number, taking it as a parameter so
the shapes worth testing (`500`, `-1`, `five`) can be asked of it at all; both callers,
the settings form and `builder.php`, use the no-argument form, so they cannot disagree

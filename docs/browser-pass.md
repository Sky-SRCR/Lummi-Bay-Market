...
**And v2 has since rewritten most of what it describes.** Nine debts have accumulated,
...
| A walk of the **canvas against a television**, side by side | §4bx is three defects that only a second screen can show: a text block drawn in the interface's font instead of the Brand's, a text box 8px narrower than the sign's, and every in-section block 2px off. All three are fixed and all three were **invisible to this repo** — the container has neither Arial nor Segoe UI, so both pages substitute the same font and measure identically, and invariant 42 can only read the stylesheet. **Put the Builder and the Viewer on the same layout, on a machine with real fonts, and compare where every line wraps.** The blocks to distrust are the ones sized to *just* fit their text | §4bx |
| A walk of a **marquee, watched** | §4bz replaced the marquee's loop: the message is now repeated across one strip that snaps back by one unit, instead of one span reset when the tail cleared the far edge — so the interval is the gap somebody typed and no longer the width of the sign. The suite can drive frames and assert the arithmetic; it cannot see a **seam**. **Put a marquee on a real television at the speeds the shop actually uses and watch it for a minute.** What to look for: whether the snap is invisible, whether a gap of `0` reads as one continuous sentence rather than two words run together, whether the first pass entering from the far edge looks deliberate, and whether a sign left running overnight is still in step in the morning | §4bz |
...
| A walk of **Workspace Themes** — the gear's picker, and Site Branding's theme form | step 5 gave the application colours of its own. A suite can prove that switching sets sixteen custom properties and loses no unpublished work; it cannot see whether the picker card fits inside the gear menu at 1080p, or whether the contrast warning wraps. **Make a theme with a light work area, choose it, and read every banner in the Builder.** Then make one whose sixteen colours are identical and check you can still find the gear and get back to the store default. **Narrowed on 2026-08-21** (§4bv): the light-theme half of that was driven in a real Chromium against a real MariaDB — a theme with a light nav, work area and panel, and dark text roles, worn by a real account. The Builder's palette captions, its inspector, its canvas footer and Help's side navigation all read; the gear menu, the modals and the rail's hint lines do too. What is still owed here is what that browser was not: 1080p rather than 1280×820, the picker card inside the gear menu, a warning long enough to wrap, and the *raised chips* — the palette's buttons and Help's feature cards keep literal dark backgrounds by design, and whether a light rail with dark chips on it reads as deliberate is a question for eyes | §4bf |
...
| A walk of **the one-file `install.php` over a real FTP client** | §4bo made the install one file you upload and open, and CI now builds the database half from nothing on every leg — the server, nine tables, convergence, the credentials file, the administrator, the venue, and a second one refused. **None of that has met a browser or an FTP client.** The specific things a machine here cannot ask: whether a client in ASCII mode really does corrupt the payload and whether the sentence about binary mode is the one somebody acts on; whether the preflight's ✕ and ⚠ read as different things at a glance; whether the credentials really land above the webroot on a host whose layout is not this one's; and whether `install.php` is a **404** afterwards rather than a page saying it could not delete itself. Do it on a folder with nothing in it, against a database with nothing in it. **Then do it a second time, in a second folder on the same account** (§4bp): that install should refuse to adopt the first one's credentials file and ask for a database of its own, and the sentence it prints is the thing to read — it was silently reporting *Installed* against the first install's signs, which is how §4bp was found. **And on the first walk, fill in the *your store* fieldset** (§4bq): a real logo through a real file picker is the one line of that path a suite cannot reach — `move_uploaded_file()` needs a genuine multipart request — so what to look for is the logo appearing in three places afterwards (the sign-in page, the Asset Library as *Store logo*, and the venue's Brand), and the admin bar drawn in the colour that was typed with legible text on it. **Narrowed on 2026-08-21**: a MariaDB was put on the build container and the whole install was driven over HTTP with `curl` — both forms, a real multipart upload, `move_uploaded_file()` included. Tables made, one Display seeded, `uploads/install_<hex>.png` written with nothing of the sent filename in it, the asset row, the Brand carrying it, `branding_config.php` holding all six settings with the text colour derived, one administrator, and `install.php` a 404 afterwards. Both refusals too: a PHP file named `.png` was refused by its type with **no `uploads/` folder created and no account made**, and `nav_bg=greenish` was refused naming where that colour shows. So what is still owed is only what a machine here cannot be: a real host, an FTP client in the wrong mode, a real certificate, a cPanel database form, a browser's own file picker, and a person reading ten steps to see whether they are followable | §4bo, §4bp, §4bq |

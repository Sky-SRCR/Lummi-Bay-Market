...
| 44 | Nothing set a timezone, so "editing since 2:15pm" followed whatever the host's `php.ini` happened to say. | A store timezone setting on the Branding page. | **Done** — the setting is a tenth `branding_config.php` entry, on the Settings tab rather than beside the four colour pickers, and it takes a zone *name*: `+08:00` and `PST` both build a valid `DateTimeZone` and are both wrong for half the year, which is this same defect with a smaller error bar. There were **three** clocks, not one — PHP's process zone, unset on the live host and therefore UTC; MySQL's session zone, which `CURRENT_TIMESTAMP` used and nothing had ever set; and the store's, nowhere in the repo. Fixing the first without the other two would have left the sentence wrong by a different amount. **And a defect nobody had asked about:** the rule "a stored moment is UTC" was written out three times and the third copy left the `' UTC'` off — `lastPublishDescription()`, the one sentence a refused publish shows to say whose work is about to be walked over. It was wrong at both ends at once and the two errors cancelled on a host where MySQL and PHP agreed, which is why neither engine's fixture could see it. Reading a stamp is one place now (invariant 28). | §4ap |

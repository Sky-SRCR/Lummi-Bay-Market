...
- **#50 closed by becoming a rule.** Its first half asked for a hand count to be swept,
  ...
  invariant 30. **How much of `lib/` has been swept is a question for the tool, not for
  this line** — `php tools/mutate.php --swept` answers it, and this file carried a wrong
  number for it until 2026-08-13 (§4aq has how three documents came to hold three
  different ones). The rest are a command each, and running that command is now part of
  writing a check rather than a task somebody finishes. Reading this line as "coverage is done" is the exact mistake
  #50 was filed about.
...
Step 4 also exposed a hole, and it is now closed. The container these sessions run in has
PHP 8.4, so `php -l` — §5's first line — cannot see a construct that is fine there and a
parse error at 8.2, and a parse error in a file a Screen loads is a blank sign rather than
a message. That is the shape #50 was filed about, a gate that cannot fail on the thing it
is for, and it was true of this repo's oldest check. **Invariant 31** decides it instead,
over real tokens rather than text (§4ar). What remains by eye is that a denylist cannot
know what 8.5 adds — printed on every run, because a denylist matching nothing looks

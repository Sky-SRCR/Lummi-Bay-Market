...
// The file is rewritten whole from these arguments, so every caller passes every
// setting — the ones it is changing and the `$cur*` it is not. A caller that
// leaves one out does not leave it alone, it resets it to whatever the parameter
// defaults to, which is the same silence the grant matrix was saving over
// (CLAUDE.md, "a form that submits state must say what it covered"). Adding a
// setting here means visiting both call sites below.
function writeBrandingConfig(string $logo, string $navBg, string $navBorder,
                             string $accent, string $navText,
                             string $siteName, string $mailFrom, string $mailName,
                             int $undoSteps): bool {
    ...
    $c .= "define('UNDO_STEPS',       " . var_export($undoSteps, true) . ");\n";
...
// Through the helper, not the raw constant: config.php is the one place that
// decides what a stored undo depth means, and the Builder reads it the same way.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action_save_branding'])) {
        ...
        if ($msg === '') {
            if (writeBrandingConfig($logoPath, $navBg, $navBorder, $accent, $navText, $curSite, $curMail, $curMailN, $curUndo)) {
    ...
    if (isset($_POST['action_save_settings'])) {
        ...
        // 0 is a real answer here — it switches Undo off — so this one cannot use
        // the `?:` the three above use, which would read 0 as "not filled in" and
        // quietly keep the old depth. A field that was never on the form at all is
        // the different case, and keeps what is stored.
        $undoSteps = isset($_POST['undo_steps'])
            ? max(0, min(UNDO_STEPS_MAX, intval($_POST['undo_steps'])))
            : $curUndo;
        if (writeBrandingConfig($curLogo, $curNavBg, $curBorder, $curAccent, $curText, $siteName, $mailFrom, $mailName, $undoSteps)) {
            $curSite = $siteName; $curMail = $mailFrom; $curMailN = $mailName; $curUndo = $undoSteps;
            $msg = 'Settings saved.';
        } else {
            $msg = 'Could not write branding_config.php. Check file permissions.'; $msgType = 'error';
        }

...
| `tools/selftest_layout.php` | `php tools/selftest_layout.php` — real modules, in-memory SQLite, **1508 checks** — and the same suite against real MySQL when `SELFTEST_MYSQL_DSN` is set, where it runs 1531. Run before pushing |
...
- **Nothing here has run against MySQL or in a browser.** Verification so far is
  `php -l`, 1508 self-test checks against SQLite, 318 node checks over `builder.php`'s and `viewer.php`'s

...
| **The installer package** · §4bn | **Nothing to do for this sign, and one thing not to do.** This is for an *empty* database: `php tools/build_installer.php` writes `dist/store-display-system-<commit>.zip`, holding `app/` (48 files), `INSTALL.md`, `VERSION.txt`, `MANIFEST.txt` and a fill-in-the-blanks credentials file for the folder above the webroot. **Do not unpack it over `lbm/` or `lbm-test/`** — it carries `setup.php` and the repo's default `branding_config.php`, which are exactly the two files group A and group B of [`docs/DEPLOY-SKIP.md`](docs/DEPLOY-SKIP.md) say must never go over a running install. For an update, keep doing what that page says. | A repo is not a package: 160 tracked paths, 48 of which belong in a webroot, and until now the list saying which was a person reading DEPLOY-SKIP at an FTP client. `MANIFEST.txt` is the part worth knowing about even here — every file with its SHA-256, which is how an upload that dropped one gets found instead of guessed at, and the mistake it is for (an FTP client skipping dotfiles, so `lib/.htaccess` never arrives) is one this repo has already had to check for twice. `tools/` is deliberately **not** in the package; if the app's MariaDB note ever asks for `rehearse_phase1.php`, that comes from a checkout. **Nobody has installed from it yet** — the checks are over a file list, not a walked install, so `INSTALL.md` is an owed browser-pass walk on a host with nothing on it. |
| **One file, and setup.php is gone** · §4bo | **One thing to know and one thing not to do.** `install.php` now does the whole install — it carries the app inside itself, writes the credentials above the webroot, runs `schema.sql`, converges, names the venue and creates the first administrator, then deletes itself. `setup.php` **no longer exists in the repo**, so any instruction that names it means this file now; every rule in [`docs/DEPLOY-SKIP.md`](docs/DEPLOY-SKIP.md) group B applies to it unchanged. **Do not upload it to `lbm/` or `lbm-test/`** — both already have accounts, so it would self-disable and delete itself on the first request that reached it, which is a safety net and not permission. The check at the server stays the same shape: ask for the first-administrator page and expect a 404. | A repo is not a package and a package is not an install (§4bn, §4bo). Two things worth carrying to the visit even though neither touches this sign. First, `schema.sql` seeds **no Display** — the first sign comes from `seedLegacyDisplay()` in convergence, on the first authenticated request — which is why the installer converges before it says it is finished, and is worth knowing if a restored database ever comes up with no sign on it. Second, `sqlStatements()` had two implementations, and the weaker one was in `tools/test_fixture.php`, which is what builds the MySQL fixture from `schema.sql`: it read a doubled `''` inside a string as the end of it. There is one now, in `lib/schema.php`. CI installs from nothing on every MySQL leg (`tools/rehearse_install.php`); the browser and FTP half is an owed walk. |
| **Which database an install is on** · §4bp | **You will notice one new sentence, on a card, and it is about `lbm-test/`.** Settings → This Server has always named the install folder and the database; the row now carries a note when the folder has **no credentials file of its own**. `lbm/` reads the shared file and will carry the note — correct, and nothing to do. **`lbm-test/` is the one to read.** No note there means `private/db_credentials_lbm-test.php` exists and the rehearsal copy is isolated; a note there means it does not, and that folder is the live sign with a different URL — which is exactly what that card read on 2026-08-11 (§4ap, and the finding above), and the repo has never recorded the file being created. Nothing to click either way; the note is the answer, and the **Database** row below it is the confirmation. | A second install used to connect through the shared credentials file, find an administrator in the first install's database, print *Installed* and delete itself — the read side of the isolation rule did not exist, while the write side has been right since the day it landed. Credentials files the installer writes now carry `DB_INSTALL_FOLDER`, and one naming a different folder is refused rather than adopted. Every file written **before** this build has no stamp, including the live one, so the behaviour there is unchanged and the note on the card is the whole of what is new. `INSTALL.md` gained **Pointing an install at a different database**, which is the answer to "it connected to the wrong one": one file above the webroot, and re-upload `install.php` if the new database is empty. |
| **The install form asks about branding** · §4bq | **Nothing to do for this sign — and one thing that is now findable rather than remembered.** The installer's administrator form gained an optional *your store* fieldset: store name, the address mail is sent from, a logo file, and two colours. It changes nothing about an install that already exists, and it is not for `lbm/` or `lbm-test/` (both have accounts, so `install.php` self-disables there — see the §4bo row). What is worth carrying: **the mail-from address is now asked at install time**, so a future install of this app somewhere else does not repeat the one default whose failure suppresses its own alert. Admin Panel → Site Branding is still where it is changed, and still worth reading back after any install. | Everything a customer sees lives on four signed-in pages, which is right for a palette that wants a preview and wrong for the four facts a person installing already has in hand. The order inside the write is what decides what a failure leaves behind — venue, store details, account, because the account is what switches the installer off — and the logo is moved only after every other refusal has been ruled out, since `move_uploaded_file()` cannot be rolled back. Invariant 40 came out of it and named a **fourth** upload door nobody had listed (`admin_panel.php`'s Site Branding logo), which turned out to be the strongest and is now the shape the installer copies. It also found why **CI's five MySQL legs had been red since `38ec68c`**, which matters here rather than only in the write-up: **the installer could not install.** `applySchemaScript()` declared its out-parameter `array &$failures = []`, both real callers passed an uninitialised variable — the shape that default invites — and an undefined variable passed by reference is `null`, which the type rejects. A fatal `TypeError` on the statement that makes the tables. No local gate saw it because that function had **no checks at all**; its only other caller needs a MySQL server. So any install attempted from a package built between `38ec68c` and this commit stops at the database step — **rebuild the package before installing anywhere.** A second defect was hiding behind it: the rehearsal's own check anchor said 33 where 35 was true, never reached. Both fixed, and a MariaDB was put on the build container to prove it: 42 rehearsal checks, 2520 MySQL layout checks, `rehearse_phase1` clean — the first green run of anything in this project on MariaDB, which this file elsewhere calls untested. **And then the whole install was walked over HTTP** with `curl` against that database — both forms, a real multipart upload — so `move_uploaded_file()`, the asset row, the Brand's logo, all six branding settings and the self-delete are no longer theory. Both refusals were walked too: a PHP file named `.png` refused by its type with no `uploads/` folder created and no account made, and a colour that is not one refused by name. What is still owed at the venue is only what this container cannot be — a real host, an FTP client, a certificate and a cPanel database form. |
| ~~**The installer asks whose database it is**~~ · §4br — **superseded by §4bt, do not follow this row** | **This is the fix for what happened on `drive-thru/`, and it needs a rebuilt package to be any use.** A folder with no credentials file of its own, whose shared file reaches a database that **already has an administrator**, no longer gets *Installed* — the installer stops on *Which database does this folder use?* and offers two answers: write down that this folder uses the database it found (one button), or give it its own (the ordinary database form, plus the current database's password as proof). **Nothing to do on `lbm/` or `lbm-test/`**, and still nothing to upload there. For the next install: **rebuild the package first** — an older `install.php` will silently do the old thing. If you would rather not go through the installer at all, *Pointing an install at a different database* in `INSTALL.md` is one file above the webroot and has not changed. | §4bp made the file say which folder it was written for and left the unstamped case — every file written before it, including the live one — behaving as it always had, with a sentence. The sentence was printed and it was the only thing that happened: the folder had already been told it was finished and the installer had already deleted itself, so the first reader was the server card, days later, on an app serving another install's signs. The assumption that made adopting reasonable is the one the app itself refutes: **an install that is finished has no `install.php`**, so for that page to run at all somebody put it there on purpose, and what they are doing is installing that folder. The new field on the "own database" answer is the guard that replaces the old self-delete — pointing a live app at a different database is a takeover if anybody can do it, and the password of the database in use is the one fact the page can check and cannot leak. A blank stored password is a refusal, not a pass. Walked over HTTP both ways on MariaDB: a wrong password wrote nothing at all, the right one built nine tables in the new database and left the first one's administrator untouched, and the one-button answer left a re-uploaded `install.php` no longer asking. |
| **The installer will not use another install's credentials file at all** · §4bt | **This is the row to follow for `drive-thru`, and it replaces the two above it.** A folder is only ever connected through a credentials file that can be shown to be its own: the one named after the folder, or the shared one carrying that folder's stamp. Anything else is refused — nothing connects through it, **that database's name appears nowhere on the page**, no password for it is asked for, and the ordinary database form is offered for a database of the folder's own. **Needs the rebuilt package.** For a new install: create the empty database, upload `install.php`, fill the form in. **Nothing to do on `lbm/` or `lbm-test/`** — but note the cost below before putting `install.php` into a folder that is already working. | §4bp adopted an unstamped shared file and printed a warning that reached nobody until the installer had deleted itself; §4br asked which database the folder used, and one of its two answers was still to adopt while the other asked for the password of the database in use. Both were the page settling a question it has no facts for. **The cost of refusing:** an install whose credentials really are in the unstamped shared `/home1/silveradmin/private/db_credentials.php` will no longer recognise itself if `install.php` is put back into its folder — it gets the database form instead of *Installed*. The way in is one line added to that file, `define('DB_INSTALL_FOLDER', 'lbm');` for `public_html/lbm/`, and the screen spells it. Worth doing on that file now, before the next install, whether or not you ever re-upload an installer. Walked on MariaDB: the old database was never named on the page and ended the walk with the same 9 tables, 1 administrator and 1 Display it started with. |
| **The last screen says whether it made you an account** · §4bu | **Nothing to do, and it answers a question worth having the answer to.** The installer asks for the username and the password on its **Administrator** step, and always did — nothing in this app has ever generated or assumed an account, and `schema.sql` seeds none. What it *also* does is skip that step entirely when the database it opens already holds accounts, which is the safety net that makes a re-uploaded `install.php` harmless on `lbm/`. Until now both endings printed **Installed** and a sign-in link. Now the second one says *This database was already installed*, that no account was created here and nothing was asked for, and how many of the accounts already in there can sign in as an administrator. **Needs the rebuilt package** to be visible, like everything else on this list. | So an administrator in a database that nobody remembers creating is not a mystery and not a default: it is an account an earlier run of this installer — or the old `setup.php`, or a hand-made row — put there, and the folder that came second was told the install was finished without being told it had installed nothing. The new third case is the one nobody would build on purpose and any restored database can arrive in: accounts in there, none of which is an administrator who can sign in (closed, suspended, or `basic`). That used to get *Installed* and a link to a sign-in page that cannot let anybody in; it is now a refusal naming what to fix. The installer still will not create an account in a database that already holds them — that form has no account behind it, so over live data it is an administrator for whoever finds the file. |
| **A theme can set the text colour too** · §4bv | **One thing to know and one thing that was already there.** A Workspace Theme now has three more colours: **Text on panels**, **Text on the work area** and **Text on buttons and banners** — so a theme with a light rail no longer leaves the palette's own labels grey on grey. They default to exactly the colours those places already were, so every theme anybody has made looks the same as it did. The **font colour of a block** is a different thing and was always editable: **Display Branding → Brand Standards**, per block type, per Brand — it is what the television shows, so it belongs to the venue's Brand and not to one employee's theme. The theme form now says so under its preview. | The three columns are added by convergence on the first signed-in page load (`workspace_themes.panel_text` and its two siblings), so nothing needs running by hand and no existing theme moves. Two things worth knowing. **The contrast warning now checks nine text-on-surface pairs instead of one, and two of them failed on the store's own defaults**: white on the blue `#3498db` was 3.2:1 and on the green `#27ae60` 2.9:1, where 4.5:1 is the readable minimum. Both were nudged on the owner's say-so — see the §4bw row below, which includes the one thing that needs doing on `lbm/` by hand. And the **raised chips** are still literal: the palette's buttons, the Brand control and Help's feature cards keep their dark backgrounds whatever the theme, so a light theme has dark chips on a light rail. Deliberate for now, and named in §4bv as the limit. |
| **The blue and the green are a shade darker** · §4bw | **One thing to do on `lbm/`, and it takes ten seconds.** The Publish-button blue moved from `#3498db` to `#207ab6` and the *Saved* green from `#27ae60` to `#1e8449`, so white text on them is readable (4.65:1 and 4.72:1, against 3.15 and 2.87 before). The **green reaches you on the first page load** with no action. The **blue does not**: the accent is one of the four colours `branding_config.php` holds, and that file on the server is yours and is never overwritten — so open **Admin Panel → Site Branding**, set the accent to `#207ab6`, and save. A new install gets it from the package. | The trade-off is stated rather than buried, because it cannot be avoided: a filled button wants a readable label (4.5:1) *and* a findable edge against what is behind it (3:1), and on the dark work area no colour can do both — the label needs a luminance under 0.183 and the edge needs over 0.273. The old blue met the edge bar and failed the label; the new one is the other way round. A filled button with white words on it is identified by its words, so the label won. What you will notice: the Publish button and the green buttons sit a little quieter against the dark background. Both colours are the *shallowest* darkening that clears 4.5:1 on the same hue, so it is the smallest version of that cost. |
| **The Builder now draws what the sign draws** · §4bx | **Three fixes to the canvas, no action needed, and they change what you see.** Text blocks were drawn in the *interface's* font rather than the Brand's, so a block that fitted on one line in the Builder could wrap on the television — that is the two screenshots. The text box was also 8px narrower than the sign's and its first line 4px lower. And every block **inside a section** was drawn 2px right and 2px down of where the sign draws it (3px while the section was clicked), which is why *Center in parent* looked 2–3px off to the right. After this the Builder and the Viewer measure identically. Existing layouts are untouched — nothing was stored wrong, it was only ever drawn wrong. | The section's purple edge was a `border`, and a bordered absolutely-positioned box holds its children at its *padding* box — so the children moved by the border's width while the code that positions them, which measures the border box, was right. It is an `outline` now: painted, takes no space, the same thing the blocks themselves had always used. The font one was a cascade trap: the Brand's family sits on the block and the text sits in a child, so `* { font-family: … }` matched the child **directly** and beat the inherited value. `body` carries the interface font now. Worth knowing that **this container cannot see that defect at all** — it has neither Arial nor Segoe UI, so both pages substitute the same font and measure the same; it is only visible where the fonts exist, which is every machine in the shop. Invariant 42 guards all three by reading the stylesheet rather than the rendering. |
| **Rounded corners, and table rows that move** · §4by | **Two new things in the Builder, nothing to run.** Every block except a text block now has a **Corner Radius** box in the rail — pixels, 0 for square — and it rounds the picture inside the block as well as the block, on the Builder and on the television. Sections have it too. Text blocks do not, because a text block paints no box for a radius to round; its *font* colour is still Brand Standards. And in **Edit Table**, every row has ▲▼ arrows in a narrow column on the left, disabled at the top and bottom. They move a row one place and carry whatever you have typed into it, saved or not. | The column (`canvas_elements.corner_radius`) is added by convergence on the first signed-in page load and defaults to 0, so no sign that is already up changes shape. Verified against a database built from the previous commit's schema: the column arrives, the existing rows keep their geometry exactly, and the second page load asks for no further change. Rows are reordered by arrows rather than by dragging on purpose: every cell in a table row is a text input, so a drag needs a handle to start from, which is the same two clicks the arrows are — and the arrows can say when you have reached the end of the list. Say the word if you would rather drag; it is a larger change for the same result. |
| **A marquee that comes round on a clock** · §4bz | **New box in the rail, and it changes how every marquee already up behaves.** A marquee's message used to come round again only after its tail had cleared the far edge of the sign — so on a 1920-wide board at the default speed, a four-second message repeated every twenty-eight seconds, and nothing on any form could shorten it. The **Restart Gap** box under Scroll Speed is that wait, in seconds: the message goes by, then that many seconds of blank, then it goes by again. `0` is nose to tail, 60 is a full minute of quiet bar. Every marquee published before today gets **2 seconds**, which is a change you will see without touching anything — set it to what you want per marquee and Publish. | The message is now repeated across one strip and the strip is snapped back by exactly one message-plus-gap, which is invisible because every copy is identical — so the interval is `message length ÷ speed + gap` with the block's width nowhere in it. Two seconds is a chosen default rather than a fallback: a marquee saved before this carries no gap at all, and nose-to-tail is not what those signs meant. Nothing here has been *watched* on a television, and that walk is on the browser-pass list. | 
| **A folder whose credentials are already written gets a button, not a form** · §4bs | **This is the screen you meet if you place the credentials file yourself, which is what *Pointing an install at a different database* in `INSTALL.md` tells you to do.** A clean folder, a credentials file above the webroot that works, and an empty database behind it now reaches a screen that **names that file and that database** and offers one button — *Create the tables*. It writes nothing above the webroot, so the file you placed is left byte for byte as you wrote it. The four-field form is still there under *Or use a different database*, for a file that names the wrong one. **Needs the rebuilt package**; an older `install.php` shows the form. **Nothing to do on `lbm/` or `lbm-test/`.** | Until this, that state — the recommended manual route — reached the **blank database form**: a page holding the file's name, the database's name and a live connection, asking for all four values again. And `installerDoDatabase()` rewrites the credentials file from whatever is typed, so the route with the fewest moving parts had the only screen that could overwrite the working file it had just read, from one typo. Walked on MariaDB: nine tables, the Display seeded, the administrator form, and both credentials files untouched. |
...
**And a second reading, 2026-08-21, from a folder called `drive-thru/`.** Not a repeat of
the one above: it is a *different folder*, on a newer build of the app, and four of its rows
say something the 2026-08-11 card could not.

| Row | Value | What is new about it |
|-----|-------|----------------------|
| PHP version | **8.3.33** | **Above** the 8.2.33 recorded above and above the `ea-php82` pin §7 describes. Nothing breaks — 8.2 is a floor, not a ceiling — but the pin is per-domain in MultiPHP Manager, so this folder is not covered by the one that was read. The floor holds; the *observation* that the host serves 8.2 does not hold for every folder on it. |
| MySQL version | 5.7.23-23 | Third reading, unchanged, and exactly the declared floor. |
| Store time zone | `America/Los_Angeles` | The shipped default, so it has never been changed on that install — which is right for this store (§4ap). |
| PHP time zone | `America/Los_Angeles` | `config.php` setting the process default, working. |
| **Database time zone** | **`+00:00`** | **The one that had never been read.** §4ap gave `db_connect.php` a suppressed `SET time_zone = '+00:00'` and said the card was the only place a refusal would show. It was not refused. |
| Session cookie | HttpOnly yes, **Secure yes**, SameSite Lax | `RequestScheme::isSecure()` deciding correctly over a real certificate — the flag whose flat version was a correct password landing back on a blank form for ever. |
| Largest upload | 50 MB | Unchanged. |
| This install | `drive-thru`, **no credentials file of its own** | §4bp's note, in the field, on a third folder — saying that folder reads the shared `db_credentials.php` and naming the database it reached. Correct, and the reason §4br exists. |

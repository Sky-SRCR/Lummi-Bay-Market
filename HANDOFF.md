...
| `lib/install_paths.php` | Which install this folder is, and whose credentials it uses. Pure. One account can hold the live app and a rehearsal copy at the same depth, so a single shared credentials path made an unmodified copy connect to the **live** database in silence — the folder's own name selects `private/db_credentials_<folder>.php` when it exists, and the shared file otherwise, so no tracked file has to differ between the two. The private directory is **searched for, nearest ancestor first, not counted two levels up** (§4bn): the old arithmetic was right for `public_html/lbm/` and resolved above the account from a subdomain docroot one level down, where the app could not connect at all. Two names are tried, the parent-qualified one first (`db_credentials_lbm.srcresort.com-lbm.php` before `db_credentials_lbm.php`), because one account can hold two folders with one basename and the live sign is one of them |
...
| `tools/selftest_layout.php` | `php tools/selftest_layout.php` — real modules, in-memory SQLite, **2354 checks** — and the same suite against real MySQL when `SELFTEST_MYSQL_DSN` is set, where it runs 2379 (that figure is the SQLite one plus a count of the engine-only section; see the note above `reportChecks()`). Run before pushing |
...
| **A second front door on its own subdomain** · §4bn | **Two steps, and neither is in the app.** (1) `lbm.srcresort.com` **must be given a DNS record at Cloudflare** — `srcresort.com`'s nameservers are `elisabeth.ns.cloudflare.com` and `tony.ns.cloudflare.com`, so the zone cPanel writes when it creates a subdomain is not the zone the internet reads, and the name answers NXDOMAIN until an `A` record pointing at the account's server IP exists there. Copy the IP off the existing `srcresort.com` record rather than trusting one written down; start it **DNS only** so cPanel's AutoSSL can issue for the new hostname, then proxy it if you want. (2) **The document root may now be at any depth** — that is what §4bn changed — but if two installs end up with the same folder name (`public_html/lbm/` and `lbm.srcresort.com/lbm/`), the file that isolates one is `private/db_credentials_lbm.srcresort.com-lbm.php`, **never** `private/db_credentials_lbm.php`, which both would read. | The credentials lookup counted two directories up, so a docroot one level below the account resolved above the account entirely: no credentials, the placeholders in `db_connect.php`, and no connection. Fixed, and the live install's answer is unchanged and asserted. Nothing is installed on that subdomain yet, and nobody has walked a browser over it — `docs/browser-pass.md` owes that walk. |

...
| `tools/page_constants.js` | Not run on its own. The one place that says what the server puts on a page — the twenty-one values `builder.php` interpolates and the three `viewer.php` does — so a node suite that does not override one is running the page rather than a version of it where that value is the literal 0. Refuses a constant the page has that nothing here names, and an override for a constant the page does not have (§4bf) |
| `tools/check_invariants.php` | `php tools/check_invariants.php` — the mechanical greps from BUILD-REFERENCE §5, run as pass/fail against the whole file set rather than a count, with comments dropped so prose about a rule does not fail it. **73 checks**, anchored — a rule deleted from the list is a failure, not a smaller green number (§4bg). Prints what it deliberately does not cover on every run |
| `tools/check_doc_numbering.php` | `php tools/check_doc_numbering.php` — no two write-ups share a number, no citation dangles, the invariants run unbroken, and **the next free section letter**, which is the question every branch cut from the same base has to answer. **6 checks**, anchored (§4bg) |
...
| **v2 step 1** · §4az | **Nothing to do, and it is the safe one to send first** — no schema, and nothing anybody can see changes. Two things to expect anyway. **A Builder tab left open across the deploy still publishes fine**: it goes on sending the brand-owned typography fields, and the server *ignores* them rather than refusing, because the person publishing did not know they were sending a field and refusing their afternoon's work over it would be this app inventing a problem. And **the colour audit gets quieter**: an unreadable colour on a *branded* block is now reported once, against the one `block_styles` row it lives in, instead of once per price per sign. | Publish used to bake the shared typography standards into every branded element's own `font_*` columns — invisible while every Display shared one standard, a live fault the moment two Brands exist. Rows published before this keep those baked values; they are inert while the shop has one Brand, and each Display's next publish clears its own. If a second Brand is created before a sign is republished, changing a block's subtype can show the old Brand's typography — republish that sign and it is gone. `copyLayout()` asks the same question, so duplicating an old layout does not carry the fossil forward. It is invariant 32. |
| **v2 step 2** · §4ba | **Nothing to do, and tell whoever edits prices before they open it.** No schema, no new data — but every screen in the Builder has moved: the add-block buttons leave the top bar for a left column, the properties panel becomes a docked right rail that cannot overlap the canvas any more, the align bar retires and its tools reappear inside that rail as **Arrange** — so somebody hunting for "Align" will not find it by that name — and the publish line moves into a canvas footer. The rail keeps a resting state ("Select a block to edit it") rather than vanishing, which is the one thing most likely to be reported as broken. | Pure chrome, which is what made it safe to land before the features that use it — step 4's Brand control sits in the slot this step created. **Nothing in this repo can see a three-column layout fail to be three columns**, so the workbench re-walk against `lbm-test/` in `docs/browser-pass.md` is owed before this reaches a sign, not after. |
...
| **v2 step 4** · §4bc | **Nothing to do, and one thing to know before anybody publishes.** **Publish now writes which Brand the sign wears.** Picking one in the Builder repaints the canvas and writes nothing; the publish is what commits it, and an invalid value refuses the *whole* publish rather than half-applying it. With one Brand on the property that is a no-op, so the ordering only matters once a second Brand exists. Expect the venue's logo and name at the top of the left column — with a chevron for admins, without one for basic accounts and read-only Builders, who should know which venue they are building for and be unable to change it. Palette swatches appear above four colour controls, and a **Venue Logo** palette item drops an image block already linked to the Brand's asset. | No schema: `displays.brand_id` arrived with step 3. Adds no invariant either, and §4bc says why. Because step 1 landed first, cycling Brands moves no undo snapshot and bakes nothing into an element row — which is the property worth checking by hand: **switch a Brand, do not publish, reload, and the sign must be on the Brand it started on.** |
| **v2 step 5** · §4bd | **Rehearse this one too, and for a reason step 3's row does not cover.** Convergence adds the `workspace_themes` table, `users.workspace_theme_id`, its index, and the foreign key `users_ibfk_1` — so it needs **CREATE**, **ALTER**, **INDEX** and **REFERENCES**, and it is the only step that alters `users`. **That is the step §7's row-format question is about**: `users.email` is a 1020-byte unique index, legal under `DYNAMIC` row format and not under `COMPACT`, and adding a foreign key can rebuild the table. Ask before deploying, not after. At deploy: **expect nothing to change and no theme to exist.** The plan said today's `branding_config.php` values would become a seeded row named *Store default*; the build decided against it, because a seeded row is a copy of that file and the first Site Branding edit makes the copy disagree with it while still being called the default. So the store default is the file plus the documented defaults and **not** a row — convergence inserts nothing and cannot repaint anybody. Every account starts with `workspace_theme_id` NULL, which is what wearing nothing means, and themes exist only once an admin makes one. | Invariant 34, and no ADR — decisions 10 and 11 of the v2 roadmap carry it, which §4bd says is deliberate: a theme is one row with no second table to be half of. The application gets colours of its own, separate from what a sign is painted in — thirteen roles, never `#builder-canvas` or anything drawn on it, which `tools/check_invariants.php` enforces. The gear menu gains a picker and Site Branding gains a theme form, so **this is the one v2 step that changes a setting about the person rather than about a sign**: `users.workspace_theme_id` is per-account, and an admin who sets a theme is not setting it for the shop. A theme somebody is wearing is refused rather than reassigned, naming them, and `users_ibfk_1` carries no `ON DELETE` clause so the database says the same thing to anything reaching it another way. |

...
| `tools/selftest_layout.php` | `php tools/selftest_layout.php` — real modules, in-memory SQLite, **2268 checks** — and the same suite against real MySQL when `SELFTEST_MYSQL_DSN` is set, where it runs 2293 (that figure is the SQLite one plus a count of the engine-only section; see the note above `reportChecks()`). Run before pushing |

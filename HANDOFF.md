...
| `tools/selftest_layout.php` | `php tools/selftest_layout.php` — real modules, in-memory SQLite, **2273 checks** — and the same suite against real MySQL when `SELFTEST_MYSQL_DSN` is set, where it runs 2298 (that figure is the SQLite one plus a count of the engine-only section; see the note above `reportChecks()`). Run before pushing |
| `tools/selftest_installed.php` | `php tools/selftest_installed.php` — the same suite three more times, as a shop that has been set up: branded, live-like, and one whose config holds a colour the app cannot read. `branding_config.php` is generated and outside the repo, so the plain run has only ever seen a fresh checkout — seven checks turned out to be asserting *this machine's* configuration, two of them that the shop's navigation is the colour the app ships with. About as long as three plain runs |

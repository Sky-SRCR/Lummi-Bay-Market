...
| **A locked section no longer takes new blocks** (browser pass step G, §4av) | **Tell whoever edits prices.** If a section on the live layout is locked and somebody is used to adding blocks inside it, they now get a refusal instead. The fix is to select the section, un-tick **Lock this block**, add, and tick it again. | Locking refused a drag and refused a resize and then accepted a new block silently — the third way of changing a locked section, and the one the lock exists to prevent. This narrows what Lock means, so `help.php` gained a *Locking a section* subsection. Watch for the corner: **if every section on a sign is locked, a basic account has nowhere to add at all**, because a basic account must aim at a section first. That is correct and it is a thing to know before locking everything. |

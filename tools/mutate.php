...
$sweptLedger = [
    ...
    // 84 mutants, 45 killed, 39 survived — the lowest score in this ledger, and the
    // reason is worth carrying rather than looking like neglect: fifteen of the
    // survivors are in `databaseTimeZone()` and `isUtcOffset()`, which SQLite cannot
    // reach at all (`@@session.time_zone` does not exist there and the catch returns
    // "not applicable"), so they are only reachable on the MySQL arm — which has never
    // run anywhere. Four more are in `sameSiteFromPath()`, a private string parser with
    // no seam. §4bg's own additions were swept separately and came back clean.
    'lib/server_report.php'   => ['§4bg'],
];

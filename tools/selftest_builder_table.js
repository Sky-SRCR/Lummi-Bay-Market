...
// An admin on a Display nobody else holds, with a canvas the size of the shop's.
//
// This suite stripped the PHP for itself until the two lanes met: it replaced every
// `<?= … ?>` with the literal `0` and then wrote three constants back by hand, which
// is exactly the shape §4bf was filed about — the eighteen it did not name were zero
// and nothing said so. `readonly`'s meta-check is what noticed, and it noticed on the
// merge rather than on either lane, because the rule and the suite were written in
// different trees on the same day.

...
// as the rows `tools/audit_colors.php` exists to find (§4w): the write path is
...
class Brand
{
    ...
    /**
     * Read `branding_config.php` if it is there and has not been read already.
     *
     * Idempotent twice over: `require_once` is keyed on the path, and the `defined`
     * guard covers the case where `config.php` got there first by a different route.
     * A missing file is not an error — a deployment that has never opened the
     * Branding page has never had one, and every accessor below answers the default.
     */
    public static function load()
    {
        $file = dirname(__DIR__) . '/branding_config.php';
        if (!defined('BRAND_NAV_BG') && file_exists($file)) {
            require_once $file;
        }
    }
